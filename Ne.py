...
class MultiSelectComboBox(QWidget):
    """Multi-select searchable dropdown widget"""
    ...
    def __init__(self, parent=None):
        ...
        self.selected_label = QLabel("No columns selected")
    ...
    def update_selected_label(self):
        """Update the label showing selected items"""
        selected = self.get_selected_items()
        if selected:
            if len(selected) <= 3:
                self.selected_label.setText(f"Selected: {', '.join(selected)}")
            else:
                self.selected_label.setText(f"Selected: {', '.join(selected[:3])}... (+{len(selected)-3} more)")
        else:
            self.selected_label.setText("No columns selected")
    
    def set_selected_items(self, items):
        """Pre-select specific items"""
        for i in range(self.list_widget.count()):
            item = self.list_widget.item(i)
            if item and item.text() in items:
...
class ConfigurationPage(QWizardPage):
    ...
    def __init__(self):
        ...
        self.join_multi_select = MultiSelectComboBox()
        self.join_multi_select.list_widget.itemChanged.connect(lambda: self.join_multi_select.update_selected_label())
        self.join_multi_select.list_widget.itemChanged.connect(lambda: self.completeChanged.emit())
        join_layout.addWidget(self.join_multi_select)
    ...
    def initializePage(self):
        ...
        if base_df is not None and compare_df is not None:
            ...
            self.join_multi_select.add_items(sorted(common_cols))
    ...
    def isComplete(self):
        return len(self.join_multi_select.get_selected_items()) > 0
    
    def get_join_columns(self):
        """Get selected join columns"""
        return self.join_multi_select.get_selected_items()
...
class SummaryPage(QWizardPage):
    ...
    def initializePage(self):
        ...
        join_cols = wizard.config_page.get_join_columns()
        ...
        <b>Join Columns:</b> {', '.join(join_cols)}<br>
...
class DataCompyGUI(QMainWindow):
    ...
    def start_wizard(self):
        ...
        if wizard.exec() == QWizard.DialogCode.Accepted:
            ...
            try:
                # Run comparison - use list if multiple columns, single string if one column
                join_param = join_cols if len(join_cols) > 1 else join_cols[0] if join_cols else None
                self.comparison = datacompy.Compare(self.base_df,
                                                    self.compare_df,
                                                    join_columns=join_param,
                                                    df1_name='Base',
                                                    df2_name='Compare')
    ...
    def add_mismatch_tab(self):
        ...
        if not self.comparison.all_mismatch().empty:
            ...
            if mismatch_rows:
                ...
                search_box.setPlaceholderText("🔍 Search in table...")
                ...
                # Connect search box to filter table
                def filter_table(text):
                    for row in range(table.rowCount()):
                        should_show = False
                        if not text:  # If search is empty, show all rows
                            should_show = True
                        else:
